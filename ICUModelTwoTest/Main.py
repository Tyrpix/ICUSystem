...
def get_diagnosis_result(filename):
    ...
    with open(filename, 'r', newline='') as f:
        ...
        patient = Patient(patient_row[1], patient_row[2], float(extracted_weight.strip() or 0))
        next(reader)
        next(reader)
        ...
        for row in reader:
            ...
            grv = row[3]
            extracted_grv = ''.join((ch if ch in '0123456789.' else '') for ch in grv)

            hourly_round_list.append(HourlyRound(day_set, int(time), float(extracted_feed.strip() or 0), int(extracted_grv.strip() or 0), row[4]))
